...
public class EncrypterTest extends TestCase {
    ...
    private static final byte[] TERMINAL_KEY = CMPClient.hexStringToByteArray("C3-5F-ED-30-8F-9C-34-F4-E7-E4-83-3C-67-67-13-EC-24-08-0C-3E-34-C8-F7-BD");
    private static final byte[] TRANSPORT_KEY = CMPClient.hexStringToByteArray("EF-B1-CF-18-5A-9E-DB-B7-0E-C7-F8-0A-E7-28-9A-EA-24-45-1A-EB-4E-09-E2-FE");
    ...
    public void testEncrypt() throws Exception {
        ...
        byte[] message = CMPClient.hexStringToByteArray(DECRYPTED_SERVER_RESPONSE_07);
        Encrypter instance = new Encrypter(TDES_TRANSFORMATION,terminalKey);
        byte[] result = instance.encrypt(message);
        System.out.println(CMPClient.byteArrayToString(result));
        assertEquals(SESSION_KEY, CMPClient.byteArrayToString(result));
    }
    ...
    public void testDecrypt() throws Exception {
       ...
       byte[] message = CMPClient.hexStringToByteArray(SERVER_RESPONSE);
        Encrypter instance = new Encrypter(TDES_TRANSFORMATION,transportKey);
        byte[] result = instance.decrypt(message);
        System.out.println(CMPClient.byteArrayToString(result));
        assertEquals(DECRYPTED_SERVER_RESPONSE, CMPClient.byteArrayToString(result));
    }
}
